...
async function handleDispatchCampaign(job) {
  try {
    ...
    if (campaign.mediaPath) {
            ...
            console.log("Body antes de getMessageOptions:", body); // Verifica se o texto está aqui

      //const options = await getMessageOptions(campaign.mediaName, filePath, body);
      const options = await getMessageOptions(campaign.mediaName, filePath, String(campaign.companyId), body); // 4 argumentos      
