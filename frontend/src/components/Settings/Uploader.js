...
const Uploader = () => {
  ...
  useEffect(() => {
    async function fetchData() {
      if (!user.super) {
        toast.error("¡No hay permiso para acceder!");
        setTimeout(() => {
          history.push(`/`)
        }, 500);
      }
...
const handleFileChange = (event) => {
  ...
  if (selectedFile && allowedTypes.includes(selectedFile.type)) {
    ...
    toast.error("¡Utilice únicamente archivos en formato PNG, ICO o SVG!");
  }
};
      ...
      <form onSubmit={handleSubmit} className={classes.fullWidth}>
          ...
          <FormControl className={classes.selectContainer}>
            ...
            >
              ...
              <MenuItem value="login">Pantalla de inicio de sesión</MenuItem>
              <MenuItem value="interno">Logotipo Interno</MenuItem>
	      <MenuItem value="logo_w">Logotipo oscuro</MenuItem>
        ...
        <Grid item xs={12} sm={12} md={12} style={{ display: 'flex' }}>
  			<FormControl className={classes.fullWidth}>
      			...
      			<input
                    ...
                    style={{ marginTop: 15, marginBottom: 15 }}
      			/>
      			{selectedFileName ? selectedFileName : 'Elija una imagen PNG'}
    			</label>
  			</FormControl>
		</Grid>
